...
internal static class CustomerData
{
	...
	static async Task GetAverageCustomerIncomeFromList()
	{
		...
		var customers = await JsonSerializer.DeserializeAsync>(gzipStream); var income = customers?.Average(c => c.AnnualIncome) ?? 0;
		var customerAvgIncomeStr = $"Average income: $ {income:N0}\nTotal Customers: {customers?.Count:N0}";
	}
	...
	{
		var faker = new Faker().RuleFor(c => [c.Name]("http://c.Name"), f => f.Name.FullName()) .RuleFor(c => c.Birthday, f => f.Date.Past(80, DateTime.Today.AddYears(-18)))
		// Age between 18 and 98
