/*
NOTE: I have no idea where this code came from. I found it randomly stored as a text file on my iPhone. 
*/
...
//Deserialize data from file as list var customers = await JsonSerializer.DeserializeAsync>(gzipStream); var income = customers?.Average(c => c.AnnualIncome) ?? 0;
...
//Deserialize data from file as enumerable var customers = JsonSerializer.DeserializeAsyncEnumerable(gzipStream);

long totalIncome = 0; var totalCustomers = 0; var stateIncomeStatistics = new HashSet();
...
static IEnumerable GenmerateData(int count)

{ var faker = new Faker() .RuleFor(c => [c.Name](http://c.Name), f => f.Name.FullName()) .RuleFor(c => c.Birthday, f => f.Date.Past(80, DateTime.Today.AddYears(-18))) // Age between 18 and 98 .RuleFor(c => c.State, f => f.Address.StateAbbr()) .RuleFor(c => c.AnnualIncome, f => f.Random.Int(20_000, 200_000)); // Income between $20k and $200k
...
//IEquatable implementation public bool Equals(Statistic other) { if (other is null) return false;
...
public static bool operator ==(Statistic left, Statistic right) => EqualityComparer.Default.Equals(left, right);
